...
export function InstitutionTopbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-ink/8 bg-paper/80 px-6 py-4 backdrop-blur-md lg:px-10 print:hidden">
      <Link href="/institution" className="flex items-center gap-2 lg:hidden">
      ...
      <div className="hidden lg:block">
        ...
        <p className="text-xs text-ink/45">{institution} · campus-wide view</p>
      </div>

      <div className="flex items-center gap-3">
        <span
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full font-display text-sm font-bold text-ink"
          style={{ background: "linear-gradient(135deg, var(--neon-pink), var(--berry-burst))" }}
        >
          {initial}
        </span>
      </div>
